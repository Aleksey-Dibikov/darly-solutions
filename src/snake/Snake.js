  ...
  return (
    <div className={s.snakeContainer}>
      ...
      >
        ...
        {game && (
          <div className={s.gameSplash} style={{ height: dim }}>
            <div>Game Over!</div>
            <button onClick={() => reset()}>Play Again</button>
          </div>
        )}
      </div>
      <div className={s.pointBar} style={{ width: dim }}>
        <h2>Counting points: {points}</h2>
      </div>
      {width <= 1024 && (
        <div style={{ width: dim, margin: 'auto' }}>
          <div>
      ...
      <div>
        <button className={style.btn} onClick={() => setActive(!isActive)}>
          {isActive ? 'PLay' : 'Pause'}
        </button>
      </div>
    </div>
